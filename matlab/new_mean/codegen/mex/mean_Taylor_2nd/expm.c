...
static emlrtRSInfo ic_emlrtRSI = { 51, "expm",
  "/usr/local/MATLAB/R2015a/toolbox/eml/lib/matlab/matfun/expm.m" };

static emlrtRSInfo jc_emlrtRSI = { 61, "expm",
  "/usr/local/MATLAB/R2015a/toolbox/eml/lib/matlab/matfun/expm.m" };

static emlrtRSInfo kc_emlrtRSI = { 174, "expm",
  "/usr/local/MATLAB/R2015a/toolbox/eml/lib/matlab/matfun/expm.m" };

static emlrtRSInfo lc_emlrtRSI = { 1, "mldivide",
...
void expm(const emlrtStack *sp, real_T A[16], real_T F[16])
{
  ...
  if (normA <= 5.3719203511481517) {
    ...
    while ((!exitg1) && (i < 5)) {
      if (normA <= theta[i]) {
        st.site = &ic_emlrtRSI;
        PadeApproximantOfDegree(&st, A, iv10[i], F);
        exitg1 = true;
      } else {
        i++;
      }
    ...
    for (j = 0; j < (int32_T)s; j++) {
      for (i = 0; i < 4; i++) {
        for (eint = 0; eint < 4; eint++) {
          b_F[i + (eint << 2)] = 0.0;
          for (i8 = 0; i8 < 4; i8++) {
            b_F[i + (eint << 2)] += F[i + (i8 << 2)] * F[i8 + (eint << 2)];
          }
        }
      }
