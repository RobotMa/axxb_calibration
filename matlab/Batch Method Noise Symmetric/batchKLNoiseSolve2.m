...
for k=1:2000
    ...
    s1=s(k)+delta_s;
    COST_temps=KLcost(s1, phi(k), SigX_solved(:,:,k), R_nanb, MeanA, SigA, thetaa, na, va, MeanB, SigB, thetab, nb, vb);
    s(k+1)=s(k)-alpha*(COST_temps-COST(k));
    
    
    
    phi1=phi(k)+delta_phi;
    COST_tempphi=KLcost(s(k), phi1, SigX_solved(:,:,k), R_nanb, MeanA, SigA, thetaa, na, va, MeanB, SigB, thetab, nb, vb);
    ...
    for i=1:21
        ...
        COST_tempsig=KLcost(s(k), phi(k), SigX1, R_nanb, MeanA, SigA, thetaa, na, va, MeanB, SigB, thetab, nb, vb);
