...
for i=1:n
    ...
    C1 = [C1;eye(9)-kron(Ra,Rb)]; % Matlab built-in Kron product solver
    C2 = [C2;eye(3)-Ra -ua]; % equation (26) and (27) from Robot Hand-eye 
                             % Calibration 
end
...
Rx=Rx*(Rx'*Rx)^(-1/2); % Orthogonalization 
% [U, ~, V] = svd(Rx);
% Rx = U*([1 0 0;0 1 0;0 0 det(U*V')])*V';


Rx=sign(det(Rx))/abs(det(Rx))^(1/3)*Rx; % Andreff Robot Hand-Eye Calibration
