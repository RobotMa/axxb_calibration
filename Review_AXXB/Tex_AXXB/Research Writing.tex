...
\author{\textbf{Qianli Ma}\thanks{Address all correspondence to this author.}
...
In the fields of robotics and computer vision, sensor calibration problems are often codified using the ``AX=XB'' formulation, (Fig.\ref{AXXBfig}). Example applications include camera calibration, Cartesian robot hand calibration, robot eye-to-hand calibration \cite{tsai1989new}, aerial vehicle sensor calibration \cite{darius1}, image guided therapy (IGT) sensor calibration \cite{Boctor-2006} and endoscopic surgery \cite{schmidt2003robust}. An alternative name that describes this system is ``hand-eye'' calibration. Several methods have existed in the literature that can perfom the hand-eye calibration without dealing with $AX=XB$ formulation such as \cite{malm2000new,wu2014hand,kim,liu2015calibration}. Camera calibration is also integrated into hand-eye calibration  in some works \cite{,,,} (to be added). Another problem called the robot-world and hand-eye calibration deals with the case where an additional calibration of the robot pose with respect to the world is needed. Multiple methods have been brought up such as \cite{zhuang1994simultaneous,dornaika1998simultaneous,hirsh2001iterative,li2010simultaneous,ernst2012non,heller2014hand}.
...
\cite{arun,chou1991finding, park1994robot,shiu1989calibration}, with the earliest proposed by Tsai \cite{tsai1989new} and Shiu \cite{shiu1989calibration}, and applications involving this problem remain active today \cite{dong,kim,dai}. Shah \cite{shah2012overview} overviewed several different AX = XB methods qualitatively. Fassi and Legnani \cite{fassi2005hand} gave a geometric view of $AX=XB$ and discussed the over-constrained and singular conditions. A more complete list of the traditional $AX=XB$ solvers includes: the Shiu method \cite{shiu1989calibration}, the screw motion method \cite{chen91,zhao2006hand}, the Euclidean Group method \cite{park1994robot,gwak2003numerical}, the quaternion method\cite{chou1991finding, horaud1995hand}, the dual quaternion method \cite{daniilidis1996dual, daniilidis1999hand}, the Kronecker method \cite{andreff1999line,andreff2001robot} and structure from motion approach \cite{schmidt2005calibration}. Several new methods emerged recently such as the convex optimization method \cite{zhao2011hand} and the global optimization method (double check with the exact methods) \cite{seo2009branch,ruland2012globally,heller2012branch,andreff2001robot,heller2014hand}.The methods mentioned previously are all off-line methods where $X$ can be calculated given a complete list of data pairs. However, several online methods are proposed that are more preferable to real application \cite{andreff1999line,angeles2000online,ackerman2014online}. All the methods above assume $\{A_i, B_i\}$ data pairs with correspondence. Probabilistic methods that deal with data pairs without correspondence are introduced in \cite{ackermanGSI, ackermanIROS2013,ackerman2014information}. 
In order to better compare the multiple existing $AX = XB$ methods, we standardize the notations and provide numerical simulations of the above methods. All of the methods mentioned in this section assumes strict correspondence between $A_{i}$ and $B_{i}$.
...
This section presents a probabilistic method to solve for $X$ wherein there does not need to be any priori knowledge of the correspondence between the exact sets of measurements
...
\section{DATA SELECTION AND ERROR METRIC}
\label{qianli-numerical}
\subsection{Data Selection}
Selection of well defined $\{A_i,B_i\}$ is very important for the $AX=XB$ solvers. Data selection methods have been proposed for off-line application in \cite{vogt2004vector,schmidt2008data}, and corresponding selection techniques for on-line solvers are introduced in \cite{shi2005approach,zhang2005adaptive}.


\subsection{Error Metrics and Covariance Matrix}
There are multiple ways to define the errors of rigid body transformation, and some methods rely heavily on the metric that is chosen \cite{strobl2006optimal}. One approach is to measure the errors of $R_{x}$ and ${\bf t}_{X}$ simultaneously. The other approach is to measure the rotation error and translation error separately. 
...
The authors would like to recognize NSF Grant RI-Medium: IIS-1162095, which served to support this work. We would also like to acknowledge the support of Kendal M. Ackerman, Alexis Cheng and Emad M. Boctor, colleagues in this endeavor at Johns Hopkins University.
\end{acknowledgment}
