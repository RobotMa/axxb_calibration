...
static void b_eml_xaxpy(int n, double a, const double x[16], int ix0, double y[4],
  int iy0);
static double b_eml_xnrm2(int n, const double x[4], int ix0);
static void c_eml_xaxpy(int n, double a, const double x[4], int ix0, double y[16],
  int iy0);
static void eml_xaxpy(int n, double a, int ix0, double y[16], int iy0);
...
double b_norm(const double x[16])
{
  ...
  for (q = 0; q < 3; q++) {
    ...
    for (kase = q + 1; kase + 1 < 5; kase++) {
      ixstart = q + (kase << 2);
      if (apply_transform) {
        eml_xaxpy(4 - q, -(eml_xdotc(4 - q, A, qs + 1, A, ixstart + 1) / A[q +
    ...
    if (q + 1 <= 2) {
      ...
      if (ztest0 == 0.0) {
        ...
        for (kase = q + 1; kase + 1 < 5; kase++) {
          b_eml_xaxpy(3 - q, e[kase], A, (q + (kase << 2)) + 2, work, q + 2);
        }

        for (kase = q + 1; kase + 1 < 5; kase++) {
          c_eml_xaxpy(3 - q, -e[kase] / e[q + 1], work, q + 2, A, (q + (kase <<
            2)) + 2);
        }
      }
    }
...
// Return Type  : double
//
double genpnorm(const creal_T x[3])
{
  double y;
  double scale;
  int k;
