...
static emlrtRSInfo kc_emlrtRSI = { 51, "expm",
  "/usr/local/MATLAB/R2015a/toolbox/eml/lib/matlab/matfun/expm.m" };

static emlrtRSInfo lc_emlrtRSI = { 61, "expm",
  "/usr/local/MATLAB/R2015a/toolbox/eml/lib/matlab/matfun/expm.m" };

static emlrtRSInfo mc_emlrtRSI = { 174, "expm",
  "/usr/local/MATLAB/R2015a/toolbox/eml/lib/matlab/matfun/expm.m" };

static emlrtRSInfo nc_emlrtRSI = { 1, "mldivide",
  "/usr/local/MATLAB/R2015a/toolbox/eml/lib/matlab/ops/mldivide.p" };

static emlrtRTEInfo m_emlrtRTEI = { 62, 5, "expm",
  "/usr/local/MATLAB/R2015a/toolbox/eml/lib/matlab/matfun/expm.m" };
...
void expm(const emlrtStack *sp, real_T A[16], real_T F[16])
{
  ...
  static const int8_T iv11[5] = { 3, 5, 7, 9, 13 };
  ...
  if (normA <= 5.3719203511481517) {
    ...
    while ((!exitg1) && (i < 5)) {
      if (normA <= theta[i]) {
        st.site = &kc_emlrtRSI;
        PadeApproximantOfDegree(&st, A, iv11[i], F);
        exitg1 = true;
      } else {
        i++;
      }
    ...
    for (j = 0; j < (int32_T)s; j++) {
      for (i = 0; i < 4; i++) {
        for (eint = 0; eint < 4; eint++) {
          b_F[i + (eint << 2)] = 0.0;
          for (i10 = 0; i10 < 4; i10++) {
            b_F[i + (eint << 2)] += F[i + (i10 << 2)] * F[i10 + (eint << 2)];
          }
        }
      }
