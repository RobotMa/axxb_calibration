...
static emlrtRTEInfo d_emlrtRTEI = { 62, 5, "expm",
  "/usr/local/MATLAB/R2015a/toolbox/eml/lib/matlab/matfun/expm.m" };
...
void expm(const emlrtStack *sp, real_T A[16], real_T F[16])
{
  ...
  static const int8_T iv17[5] = { 3, 5, 7, 9, 13 };
  ...
  if (normA <= 5.3719203511481517) {
    ...
    while ((!exitg1) && (i < 5)) {
      if (normA <= theta[i]) {
        st.site = &uc_emlrtRSI;
        PadeApproximantOfDegree(&st, A, iv17[i], F);
        exitg1 = true;
      } else {
        i++;
      }
    ...
    for (j = 0; j < (int32_T)s; j++) {
      for (i = 0; i < 4; i++) {
        for (eint = 0; eint < 4; eint++) {
          b_F[i + (eint << 2)] = 0.0;
          for (i18 = 0; i18 < 4; i18++) {
            b_F[i + (eint << 2)] += F[i + (i18 << 2)] * F[i18 + (eint << 2)];
          }
        }
      }
